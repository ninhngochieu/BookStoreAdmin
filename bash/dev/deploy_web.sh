...
if [ "$yn" == "y" ] || [ "$yn" == "Y" ] || [ "$yn" == "yes" ] || [ "$yn" == "YES" ]; then
    scp -r ./$IMAGE-$VERSION.tar centos@admin.vlaunch.aws:/tmp
    ssh -t centos@admin.vlaunch.aws "cd /tmp && sudo docker load --input $IMAGE-$VERSION.tar && rm -rf $IMAGE-$VERSION.tar"
fi

# Start web
read -p "Do you want to start web? Press y or yes to allow: " yn
if [ "$yn" == "y" ] || [ "$yn" == "Y" ] || [ "$yn" == "yes" ] || [ "$yn" == "YES" ]; then
    scp -r ./start_web.sh centos@admin.vlaunch.aws:/tmp
    ssh -t centos@admin.vlaunch.aws "cd /tmp && sudo bash start_web.sh && rm -rf start_web.sh"
fi
